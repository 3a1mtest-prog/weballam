...
صورتان، كل وحدة لمكانها:

```ts
// src/config/portfolio.ts
portrait:       { src: "/assets/hero-cutout.webp",     filter: null },
portraitPoster: { src: "/assets/portrait-poster.webp", filter: "hue-rotate(38deg) saturate(1.08)" },
```

- **`portrait`** — صورة مقصوصة بخلفية شفافة، بتوقف بين حروف `PORTFOLIO` بالغلاف.
  حجمها محكوم بارتفاع القسم مش بعرضه، عشان على الموبايل تضل متداخلة مع النص.
- **`portraitPoster`** — صورة الاستوديو بقسم WHO AM I.
- **`filter`** — أي CSS filter؛ صورة الاستوديو مضوّية بأزرق فبتلف للبنفسجي.
  خلّيها `null` لصورة أصلاً متوافقة مع القالب.

أي وحدة `src: null` يظهر مكانها لوح بنفسجي مضيء.
